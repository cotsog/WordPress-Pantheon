...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_locate_plugin() {
		...
		$this->assertEquals( 'strathcom-carousel', $location['dir_basename'] );
		$this->assertContains( 'plugins/strathcom-carousel', $location['dir_path'] );
		$this->assertContains( 'plugins/strathcom-carousel', $location['dir_url'] );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use ( $obj ) {
			$obj->assertEquals( 'StrathcomCarousel\Plugin: Param is 0!', $errstr );
