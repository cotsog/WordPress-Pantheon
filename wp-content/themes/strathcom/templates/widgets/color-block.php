 ...
 * @return string
 */
function shortcode_st_colorblock( $atts ) {
	...
	$atts = shortcode_atts( array_merge( $shortcode_atts, $macro_button_shortcode_atts ), $atts, 'st_colorblock' );
	...
	return Timber::compile( 'templates/widgets/color-block.twig', array_merge( $atts, $computed_atts ) );
}

add_shortcode( 'st_colorblock', 'shortcode_st_colorblock' );

/**
 * Defining the UI for editing the shortcode.
 */
function shortcode_ui_st_colorblock() {
	...
	shortcode_ui_register_for_shortcode( 'st_colorblock',
		array(
			'label'         => esc_html__( 'Color Block', 'strathcom' ),
...
add_action( 'register_shortcode_ui', 'shortcode_ui_st_colorblock' );
